...
simple_match_cities <- function(dfx, dfref) {
  ...
  if (!all(c("name", "code") %in% names(dfref))) {
    stop(
      "The columns 'name' and 'code' are not found in 'dfref'",
      call. = FALSE
    )
  }
...
fuzzy_match_cities_v0 <- function(dfx, dfref, dmax = 0.2, p = 0.1) {
  ...
  if (dmax < 0 | dmax > 1) {
    stop(
      "'dmax' should be between 0 (perfect match) and 1 (no match)",
      call. = FALSE
    )
  }
  ...
  if (p < 0 | p > 0.25) {
    stop(
      "'p' the prefix factor for the Jaro-Winkel distance must be between 0 and 0.25",
      call. = FALSE
    )
  }
...
fuzzy_match_cities <- function(dfx, dfref, dmax = 0.3, dist.out = FALSE) {
  ...
  d1 <- stringdist::stringdistmatrix(
    dfx$name,
    dfref$name,
    method = "jw",
    p = 0.1
  ...
  d2 <- stringdist::stringdistmatrix(
    dfx$code,
    dfref$code,
    method = "jw",
    p = 0.2
  ...
  if (dist.out) {
    out <- data.frame("match" = out, "dist" = d[cbind(1:nrow(d), out)])
  }
  return(out)
}
...
#' @export
#'
match_cities <- function(
  dfx,
  dfref,
  dfsyn = NULL,
  dmax = 0.2,
  print.out = TRUE,
  file.out = "fuzzy_match.csv"
) {
  # check arguments
  if (missing(dfx) | missing(dfref)) {
    stop("Arguments 'dfx' and 'dfref' are required", call. = FALSE)
  ...
  if (!is.null(dfsyn)) {
    ...
    if (!all(c("name", "code") %in% names(dfref))) {
      stop(
        "The columns 'name' and 'code' are not found in 'dfsyn'",
        call. = FALSE
      )
    }
    ...
    if (!all(dfsyn$id %in% dfref$id)) {
      warnings(
        "Some synonyms were not found in the reference list, they are discarded."
      )
  ...
  if (dmax > 0) {
    fm <- fuzzy_match_cities(u1, uref, dmax = dmax, dist.out = TRUE)
